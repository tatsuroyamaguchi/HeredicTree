...
def load_example_file(example_path, filename, L):
    """Load an example file from disk."""
    try:
        with open(example_path, "r", encoding="utf-8") as f:
            example_data = json.load(f)
        load_sample_data(example_data, L["success_load"].format(filename))
...
def render_sidebar_data_loader(L):
    ...
    if st.sidebar.button(L["load_example"]):
        if selected_example != "---":
            ...
            load_example_file(example_path, selected_example, L)
